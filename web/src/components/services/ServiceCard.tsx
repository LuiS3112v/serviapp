...
export default function ServiceCard({ item, detailBasePath, onQuickActionComplete }: ServiceCardProps) {
  ...
  return (
    <>
      ...
      <div
        className={`scard${isQuick ? " is-quick" : ""}`}
      ...
      >
        ...
        {isQuick && (
          <div style={{ marginTop: 14 }}>
            {!hasQuickProposals ? (
              <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 14px", background: "#F8FAFC", border: "1px solid #E2E8F0", borderRadius: 10, fontSize: 12, color: "#64748B" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {item.quickProposals!.map((p) => (
                  <div
                    ...
                    style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, padding: "12px 14px", background: "#F8FAFC", border: "1px solid #E2E8F0", borderRadius: 12 }}
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0 }}>
                      <User size={14} style={{ color: "#64748B", flexShrink: 0 }} />
                      <div style={{ minWidth: 0 }}>
                        <p style={{ fontSize: 13, fontWeight: 600, color: "#0F172A", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", margin: 0 }}>
                          {p.providerName ?? "Prestador"}
                        </p>
                        <p style={{ fontSize: 14, fontWeight: 700, color: "#0E7A5F", margin: 0 }}>{fKz(p.proposedPrice)}</p>
                      </div>
                    </div>
                    <button
