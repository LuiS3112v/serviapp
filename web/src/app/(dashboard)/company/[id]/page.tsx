...
function VerificationBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; color: string; Icon: any }> = {
    verified:  { label: "Verificada",  color: "#1D9E75", Icon: BadgeCheck  },
    pending:   { label: "Em análise",  color: "#EF9F27", Icon: Clock       },
    suspended: { label: "Suspensa",    color: "#E24B4A", Icon: ShieldAlert },
  };
...
function RequestServiceModal({
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.75)", zIndex:100, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }} onClick={onClose}>
      <div style={{ background:"#131b27", border:"1px solid #1a2535", borderRadius:20, width:"100%", maxWidth:480, maxHeight:"88vh", display:"flex", flexDirection:"column" }} onClick={e => e.stopPropagation()}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"20px 22px", borderBottom:"1px solid #1a2535" }}>
          <h2 style={{ fontSize:16, fontWeight:700, color:"#e2e8f0", margin:0 }}>Solicitar serviço</h2>
          <button onClick={onClose} style={{ background:"none", border:"none", color:"#4a6a6a", cursor:"pointer" }}><X size={18}/></button>
        </div>
        ...
        {done ? (
          <div style={{ padding:40, display:"flex", flexDirection:"column", alignItems:"center", gap:14, textAlign:"center" }}>
            ...
            <p style={{ fontSize:16, fontWeight:700, color:"#e2e8f0", margin:0 }}>Pedido enviado!</p>
            <p style={{ fontSize:13, color:"#4a6a6a", lineHeight:1.6, margin:0 }}>
              A empresa <b style={{ color:"#c0d0e0" }}>{company.name}</b> irá receber o teu pedido.
            </p>
            <button onClick={onClose} style={{ padding:"10px 24px", borderRadius:10, background:"#1D9E75", color:"white", border:"none", fontSize:13, fontWeight:700, cursor:"pointer" }}>Fechar</button>
          </div>
        ) : (
          <>
            <div style={{ padding:"20px 22px", display:"flex", flexDirection:"column", gap:14, overflowY:"auto", flex:1 }}>
              <div>
                <label style={{ fontSize:12, fontWeight:600, color:"#6a7a8a", display:"block", marginBottom:6 }}>Título do serviço *</label>
                <input style={inputStyle} placeholder={`Ex: Instalação de ${company.mainCategory}`} value={title} onChange={e => setTitle(e.target.value)} />
              </div>
              <div>
                <label style={{ fontSize:12, fontWeight:600, color:"#6a7a8a", display:"block", marginBottom:6 }}>Descrição (opcional)</label>
              ...
              <div>
                <label style={{ fontSize:12, fontWeight:600, color:"#6a7a8a", display:"block", marginBottom:6 }}>Local do serviço *</label>
                <input style={inputStyle} placeholder="Endereço" value={address} onChange={e => setAddress(e.target.value)} />
              </div>
              <div>
                <label style={{ fontSize:12, fontWeight:600, color:"#6a7a8a", display:"block", marginBottom:6 }}>Orçamento (Kz) *</label>
                <input type="number" style={inputStyle} placeholder="Ex: 25000" value={budget} onChange={e => setBudget(e.target.value)} />
              </div>
            </div>
            <div style={{ padding:"16px 22px", borderTop:"1px solid #1a2535" }}>
              <button
                disabled={!canSubmit || loading}
                onClick={handleSubmit}
                style={{
                  width:"100%", padding:13, borderRadius:11, border:"none",
                  background: canSubmit ? "linear-gradient(135deg,#1D9E75,#16876a)" : "#1a2535",
                  color: canSubmit ? "white" : "#4a5a6a",
  ...
  if (notFound || !company) return (
    <div style={{ display:"flex", minHeight:"100vh", background:"#0d1117" }}>
      ...
      <div style={{ flex:1, marginLeft:240, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:16, padding:40 }}>
        <Briefcase size={48} style={{ color:"#2a3a4a" }}/>
        <p style={{ fontSize:18, fontWeight:700, color:"#c0d0e0", margin:0 }}>Empresa não encontrada</p>
        <p style={{ fontSize:13, color:"#4a6a6a", margin:0 }}>Esta empresa pode não existir ou não estar verificada.</p>
        <button onClick={() => router.push("/search")} style={{ display:"flex", alignItems:"center", gap:8, padding:"10px 20px", borderRadius:10, background:"#131b27", border:"1px solid #1a2535", color:"#8a9ab0", cursor:"pointer", fontSize:13, fontFamily:"inherit" }}>
    ...
    <>
      <style>{`
        *{box-sizing:border-box}
        ...
        .cpub-wrap{display:flex;min-height:100vh;background:#0d1117}
        ...
        .cpub-card{background:#131b27;border:1px solid #1a2535;border-radius:18px;padding:24px}
        .sec-title{font-size:15px;font-weight:700;color:#c0d0e0;margin-bottom:14px;margin-top:0;display:flex;align-items:center;gap:8px}
        .badge-pill{display:inline-flex;align-items:center;gap:6px;padding:6px 12px;border-radius:10px;background:#0d1117;border:1px solid #1a2535;color:#c0d0e0;font-size:13px}
        .hours-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:8px}
        .hour-row{display:flex;align-items:center;justify-content:space-between;padding:8px 12px;background:#0d1117;border:1px solid #1a2535;border-radius:9px;font-size:12px}
        .cov-pills{display:flex;flex-wrap:wrap;gap:8px}
        .cov-pill{padding:5px 12px;border-radius:99px;background:#EF9F2710;border:1px solid #EF9F2730;color:#EF9F27;font-size:12px;font-weight:600}
        .btn-green{display:flex;align-items:center;gap:8px;padding:13px 24px;border-radius:12px;border:none;background:linear-gradient(135deg,#1D9E75,#16876a);color:white;font-size:14px;font-weight:700;cursor:pointer;font-family:inherit;white-space:nowrap;box-shadow:0 4px 14px rgba(29,158,117,0.3);transition:all 0.2s}
        .btn-green:hover{transform:translateY(-1px);box-shadow:0 6px 20px rgba(29,158,117,0.45)}
        .btn-chat{display:flex;align-items:center;gap:8px;padding:13px 20px;border-radius:12px;border:1px solid #378ADD40;background:#378ADD15;color:#378ADD;font-size:14px;font-weight:600;cursor:pointer;font-family:inherit;white-space:nowrap;transition:all 0.2s}
        .btn-chat:hover:not(:disabled){background:#378ADD;color:white}
        .btn-chat:disabled{opacity:0.6;cursor:not-allowed}
        .btn-back{display:flex;align-items:center;gap:8px;background:none;border:none;color:#4a6a6a;cursor:pointer;fontSize:13;font-family:inherit;width:fit-content;padding:4px 0;transition:color 0.15s}
        .btn-back:hover{color:#8a9ab0}
      ...
      <div className="cpub-wrap">
        ...
        <div className="cpub-main">
          ...
          <div className="cpub-inner">
            ...
            <div className="cpub-card">
              ...
              <div style={{
                ...
                background: company.bannerUrl
                  ...
                  : "linear-gradient(135deg,#378ADD12,#1D9E7512)",
                ...
                <div style={{
                  ...
                  background:"#1a2232", border:"3px solid #131b27",
                  ...
                  {company.logoUrl
                    ...
                    : <Building2 size={26} style={{ color:"#378ADD" }}/>
                  }
                </div>
              </div>
              ...
              <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", flexWrap:"wrap", gap:12, marginBottom:10 }}>
                <div>
                  <div style={{ display:"flex", alignItems:"center", gap:10, flexWrap:"wrap", marginBottom:6 }}>
                    <h1 style={{ fontSize:22, fontWeight:800, color:"#e2e8f0", margin:0 }}>{company.name}</h1>
                    <VerificationBadge status={company.verificationStatus}/>
                    <span style={{ fontSize:11, fontWeight:700, padding:"3px 9px", borderRadius:99, background:"#378ADD15", color:"#378ADD", border:"1px solid #378ADD30" }}>
                      🏢 Empresa
                    </span>
                  </div>
                  <div style={{ display:"flex", alignItems:"center", gap:16, flexWrap:"wrap" }}>
                    <span style={{ fontSize:13, color:"#4a6a6a", display:"flex", alignItems:"center", gap:5 }}>
                    ...
                    {company.website && (
                      <a href={company.website} target="_blank" rel="noreferrer" style={{ fontSize:13, color:"#378ADD", display:"flex", alignItems:"center", gap:5, textDecoration:"none" }}>
              ...
              {/* About */}
              {company.about && (
                <p style={{ fontSize:14, color:"#8a9ab0", lineHeight:1.7, margin:"12px 0 0" }}>
                  {company.about}
                </p>
              )}
            ...
            {services.length > 0 && (
              <div className="cpub-card">
                <p className="sec-title"><Package size={15} style={{ color:"#8B5CF6" }}/> Serviços oferecidos</p>
            ...
            {workingDays.length > 0 && (
              <div className="cpub-card">
                <p className="sec-title"><Clock size={15} style={{ color:"#378ADD" }}/> Horário de funcionamento</p>
                <div className="hours-grid">
                  {workingDays.map((h: any) => (
                    <div className="hour-row" key={h.day}>
                      <span style={{ color:"#c0d0e0", fontWeight:600 }}>{h.day}</span>
                      <span style={{ color:"#4a6a6a" }}>{h.from} – {h.to}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {(company.coverageProvinces ?? []).length > 0 && (
              <div className="cpub-card">
                <p className="sec-title"><MapPin size={15} style={{ color:"#EF9F27" }}/> Área de cobertura</p>
            ...
            {hasSocial && (
              <div className="cpub-card">
                <p className="sec-title"><Globe size={15} style={{ color:"#378ADD" }}/> Contactos e redes sociais</p>
                <div style={{ display:"flex", flexWrap:"wrap", gap:10 }}>
                  {company.socialLinks?.whatsapp && (
                    <a href={`https://wa.me/${company.socialLinks.whatsapp.replace(/\D/g,"")}`} target="_blank" rel="noreferrer"
                      style={{ display:"flex", alignItems:"center", gap:8, padding:"9px 14px", borderRadius:10, background:"#1D9E7510", border:"1px solid #1D9E7530", color:"#1D9E75", textDecoration:"none", fontSize:13, fontWeight:600 }}>
                      <MessageCircle size={14}/> WhatsApp
                    </a>
                  )}
                  {company.socialLinks?.facebook && (
                    <a href={company.socialLinks.facebook} target="_blank" rel="noreferrer"
                      style={{ display:"flex", alignItems:"center", gap:8, padding:"9px 14px", borderRadius:10, background:"#378ADD10", border:"1px solid #378ADD30", color:"#378ADD", textDecoration:"none", fontSize:13, fontWeight:600 }}>
                      Facebook
                    </a>
                  )}
                  {company.socialLinks?.instagram && (
                    <a href={company.socialLinks.instagram} target="_blank" rel="noreferrer"
                      style={{ display:"flex", alignItems:"center", gap:8, padding:"9px 14px", borderRadius:10, background:"#D4537E10", border:"1px solid #D4537E30", color:"#D4537E", textDecoration:"none", fontSize:13, fontWeight:600 }}>
                      Instagram
                    </a>
                  )}
                  ...
                  {company.socialLinks?.website && (
                    <a href={company.socialLinks.website} target="_blank" rel="noreferrer"
                      style={{ display:"flex", alignItems:"center", gap:8, padding:"9px 14px", borderRadius:10, background:"#8B5CF610", border:"1px solid #8B5CF630", color:"#8B5CF6", textDecoration:"none", fontSize:13, fontWeight:600 }}>
            ...
            {/* ── CTA final ── */}
            {isLoggedIn && (
              <div className="cpub-card" style={{ background:"linear-gradient(135deg,#1D9E7508,#378ADD08)", borderColor:"#1D9E7520" }}>
                ...
                <p style={{ fontSize:13, color:"#4a6a6a", lineHeight:1.6, margin:"0 0 16px" }}>
