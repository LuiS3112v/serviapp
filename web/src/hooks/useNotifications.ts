...
export function useNotifications() {
  ...
  // Realtime via socket — nova notificação aparece no topo da lista
  // imediatamente, sem precisar de refetch completo.
  useEffect(() => {
  ...
  // FCM foreground push — mantido como canal complementar
  useEffect(() => {
    const token = getToken();
    if (!token) return;
