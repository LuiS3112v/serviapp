...
import { Users, Briefcase, Wallet, Shield, TrendingUp, AlertCircle, RefreshCw, Check, X } from "lucide-react";
                ...
                {loading ? (
                  ...
                  pendingKyc.map(k => (
                    <div className="kyc-row" key={k.id}>
                      <div style={{ width: 36, height: 36, borderRadius: "50%", background: "#2a1e08", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: "#EF9F27", flexShrink: 0 }}>
                        {k.userName.charAt(0).toUpperCase()}
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <p style={{ fontSize: 13, fontWeight: 600, color: "#c0d0e0", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{k.userName}</p>
                        <p style={{ fontSize: 11, color: "#4a5a6a" }}>{k.documentStatus}</p>
                      </div>
                      <button className="kyc-btn" style={{ background: "#1d9e7520" }} onClick={() => approveKyc(k.id)} title="Aprovar">
                        <Check size={14} style={{ color: "#1D9E75" }} />
                      </button>
                      <button className="kyc-btn" style={{ background: "#E24B4A20" }} onClick={() => rejectKyc(k.id)} title="Rejeitar">
