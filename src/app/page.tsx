    ...
    <>
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .entry-right{display:flex;align-items:center;justify-content:center;padding:48px;overflow-y:auto;min-height:100vh}
        ...
        @media(max-width:900px){
          .entry-page{grid-template-columns:1fr;min-height:100vh}
          .entry-left{display:none}
          .entry-right{padding:32px 24px;align-items:flex-start;padding-top:40px;min-height:100vh}
        }
        @media(max-width:600px){
          .entry-right{padding:24px 16px}
          .role-grid{grid-template-columns:1fr}
          .cat-grid{grid-template-columns:repeat(2,1fr)}
          .entry-card{max-width:100%}
        }
      `}</style>
                  ...
                  <>
                    ...
                    <div className="cat-grid">
                      {["Limpeza","Climatização","Canalização","Eletricista","TI & Redes","Jardinagem","Mudanças","Beleza","Automóvel","Pintura","Construção","Segurança"].map((c,i)=>(
                        <button key={i} className={`cat-opt${providerCat===c?" sel":""}`} onClick={()=>setProviderCat(c)}>{c}</button>
                      ))}
                    </div>
